...
class JointController:
    ...
    def increment(self, joint_names, joint_points, joint, cmd_val):
        """
        ...
        :param joint_names: (list) list containing names of joints
        :param joint_points: (JointTrajectoryPoint) contains list of planned positions to be sent to action server
    ...
    def position(self, joint_names, joint_points, joint, cmd_val):
        """
        ...
        if self.has_command_value_changed(joint, cmd_val):
                joint_names.append(joint)
                joint_points.positions.append(Utils.clamp(joint, cmd_val))
                self.prev_pos_joint_cmds_map[joint] = Utils.clamp(joint, cmd_val)
    ...
    def control(self):
        ...
        for joint, joint_cmd in self.joint_commands_map.items():
            ...
            if joint == Joint.BASE_ROTATE or joint == Joint.BASE_TRANSLATE:
                ...
                if cmd_val != 0:
                    joint_names.append(joint)
                    joint_points.positions.append(cmd_val)
            else:    
                cmd_val, cmd_mode = joint_cmd.get_command_value_mode()
                if cmd_mode == CommandMode.INCREMENTAL:
                    self.increment(joint_names, joint_points, joint, cmd_val)
                elif cmd_mode == CommandMode.POSITIONAL:
                    self.position(joint_names, joint_points, joint, cmd_val)
